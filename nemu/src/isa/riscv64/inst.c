...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)               \
    ...
    //       0000100 10110 00000 000 01111 00100 11
    ...
    INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu, I, R(dest) = (uint16_t)Mr(src1 + imm, 2));
    ...
    INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu, R, R(dest) = (uint64_t)(src1 % src2));
    INSTPAT("0000001 ????? ????? 111 ????? 01110 11", remuw, R, R(dest) = (uint32_t)(src1 % src2));
    ...
    INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw, I, R(dest) = SEXT((int64_t)src1 >> BITS(imm, 4, 0), 32));
    INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll, R, R(dest) = src1 << (src2 & 0b11111));
    ...
    INSTPAT(
        ...
        bltu,
        B,
        s->dnpc = ((uint64_t)src1 < (uint64_t)src2) ? s->pc + imm : s->snpc);
    INSTPAT(
        "??????? ????? ????? 101 ????? 11000 11",
        bge,
        B,
        s->dnpc = ((int64_t)src1 >= (int64_t)src2) ? s->pc + imm : s->snpc);
    INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui, U, R(dest) = imm);
    INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu, B, s->dnpc = (src1 >= (unsigned)src2) ? s->pc + imm : s->snpc);
    // 80000080:	02e94063          	blt	s2,a4,800000a0 <main+0x78>
    //       0000001 01110 10010 100 00000 11000 11
    INSTPAT(
        "??????? ????? ????? 100 ????? 11000 11", blt, B, s->dnpc = ((int64_t)src1 < (int64_t)src2) ? s->pc + imm : s->snpc);
