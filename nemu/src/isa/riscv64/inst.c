...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */)               \
    ...
    INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw, I, R(dest) = (uint16_t)Mr(src1 + imm, 2));
    ...
    // 800001cc:	00004537          	lui	a0,0x4
    //       0000000 00000 00000 100 01010 01101 11
    ...
    //       0000000 00001 10010 001 01111 00110 11
    ...
    INSTPAT(
        "??????? ????? ????? 000 ????? 11001 11", jalr / ret / jr, I, s->dnpc = src1; if (dest == 1) {
            R(dest) = s->snpc;
            IFDEF(CONFIG_FTRACE, CALL());
        } else { IFDEF(CONFIG_FTRACE, RET()); });
    ...
    INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw, S, Mw(src1 + imm, 2, (int32_t)src2));
