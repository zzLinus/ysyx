...
static const uint8_t mbr[] = {
	...
	// GDT
	0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xff, 0xff, 0x00, 0x00, 0x00, 0x9a, 0xcf, 0x00, 0xff, 0xff,
			...
			if (vcpu.int_wp_state == STATE_INT_INST) {
				...
				*(uint32_t *)(vm.mem + eflag_addr) &= ~RFLAGS_FIX_MASK;
				...
				Assert(vcpu.entry == vcpu.kvm_run->debug.arch.pc,
				       "entry not match, right = 0x%llx, wrong = 0x%x", vcpu.kvm_run->debug.arch.pc,
				       vcpu.entry);
				kvm_set_step_mode(false, 0);
				vcpu.int_wp_state = STATE_IDLE;
...
void difftest_raise_intr(word_t NO)
{
	...
	// assume code.base = 0
	uint32_t entry = vm.mem[pgate] | (vm.mem[pgate + 1] << 8) | (vm.mem[pgate + 6] << 16) |
			 (vm.mem[pgate + 7] << 24);
